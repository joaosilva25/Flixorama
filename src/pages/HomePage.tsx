...
export default function HomePage() {
    ...
    const MovieRequest = async () => {
      ...
      try {
        ...
        const req = await fetch(
          `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=pt-BR&sort_by=popularity.desc`
        );
  ...
  const addMyList = async () => {
    ...
    try {
      const req = await fetch(
        "https://apicritiflixacesso.onrender.com/myList",
        {
          method: "PUT",
          body: JSON.stringify({
            email: email,
    ...
    <ProtectRoute>
      <div className="bg-gray-900 h-screen sm:h-full xs:overflow-x-hidden">
        ...
        <div className="bg-gray-900 h-3/4 gap-10 flex flex-col items-center">
          <h1 className="text-white font-bold mt-12 xs:text-base sm:text-xl md:text-3xl lg:text-4xl">
            O que selecionamos para você !
          </h1>
          <h2 className="text-white text-center font-light xs:text-sm xs:w-64 sm:text-sm sm:w-80 md:w-3/4 md:text-xl lg:w-1/3 lg:text-xl">
            Levando em consideração os seus gostos pessoais nos escolhemos estes
            filmes que possívelmente você adoraria conhecer
          </h2>
          <SelectedMoviesCard />
        </div>
      </div>
