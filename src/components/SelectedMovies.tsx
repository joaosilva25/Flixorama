...
const SelectedMoviesCard=()=> {
        ...
        if(userLogged) {
            ...
            const updateUserLogged=async()=> {
                ...
                if(myUserUpdated) {
                    ...
                    for(let i=0;i<myUserUpdatedToObject.userExists.myList.length;i++) {
                        setGenresCode(prev=>[...prev,myUserUpdatedToObject.userExists.myList[i].genre])
                    }
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < recommendedMovies.length; i++) {
            posters.push(`https://image.tmdb.org/t/p/original/${recommendedMovies[i].poster_path}`);
        }
    ...
    return (
        <div className="flex gap-10">
            ...
            <button className="text-white" onClick={slideToRight}>
                <FontAwesomeIcon className={`${styles.iconSlideShow}`} icon={faAngleRight} />
            </button>
        </div>
    )
}
